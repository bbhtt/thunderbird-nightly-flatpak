...
update () {
	...
	oldrelease=$(sed "${relnum}!d" "$MANIFEST_PATH"|sed -n '{s|.*/thunderbird-\(.*\)\.tar.bz2|\1|p;q;}')
	newrelease=$(wget -q --spider -S --max-redirect 0 "https://download.mozilla.org/?product=thunderbird-nightly-latest-SSL&os=linux64&lang=en-US" 2>&1 | sed -n '/Location: /{s|.*/thunderbird-\(.*\)\.tar.*|\1|p;q;}')
	sed -i "${relnum}s/$oldrelease/$newrelease/g" "$MANIFEST_PATH"
	updrelease=$(sed "${relnum}!d" "$MANIFEST_PATH"|sed -n '{s|.*/thunderbird-\(.*\)\.tar.bz2|\1|p;q;}')
	...
	wget -q https://download-installer.cdn.mozilla.net/pub/thunderbird/nightly/latest-comm-central/thunderbird-"$updrelease".checksums
	shanew=$(grep "$updrelease".tar.bz2 < thunderbird-"$updrelease".checksums|grep -v asc|grep -v sha512|cut -d " " -f1)
